...
public class EventDialog extends JDialog implements WindowListener {	
   ...
   // Add courseComboBox as a field
    public JComboBox<String> courseComboBox = new JComboBox<>();
	...
	// Add course selection panel
       JLabel lblCourse = new JLabel("Select Course:");
       gbc = new GridBagConstraints();
       gbc.gridx = 0; gbc.gridy = -1; // Set row before the repeat section
       gbc.insets = new Insets(10, 10, 5, 10);
       gbc.anchor = GridBagConstraints.WEST;
       eventPanel.add(lblCourse, gbc);
    
    // Populate courseComboBox with courses from CourseManager
       CourseManager courseManager = new CourseManager(); // Make sure this is accessible
        for (String course : courseManager.getAllCourseNames()) {
        courseComboBox.addItem(course);
      }
    
        gbc = new GridBagConstraints();
        gbc.gridx = 0; gbc.gridy = 0; // Position in the grid
        gbc.insets = new Insets(10, 10, 5, 10);
        gbc.anchor = GridBagConstraints.WEST;
         eventPanel.add(courseComboBox, gbc);
        // Build headerPanel
